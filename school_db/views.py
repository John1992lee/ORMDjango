...
def problem_one(request):
  ...
  gpa_greater_than_3 = Student.objects.filter(gpa__gt=3.0).order_by("-gpa")
  
  for greater_gpa in gpa_greater_than_3:
    print(
          f'Full name: {greater_gpa.first_name} {greater_gpa.last_name} GPA: {greater_gpa.gpa}')
...
def problem_two(request):
    ...
    for hired in instructors_hired:
        print(
            f'Instructor Full name: {hired.first_name} {hired.last_name}\nHire date: {hired.hire_date}'
        )
    return complete(request)
...
def problem_three(request):
  instructor_course = Instructor.objects.get(pk=2)
  courses = Course.objects.filter(instructor_id=2)
  print(f'Instructor Name: {instructor_course.first_name} {instructor_course.last_name}\nCourses:')
...
def problem_four(request):
    count_of_student = Student.objects.all().count()
    count_of_courses = Course.objects.all().count()
    count_of_instructor = Instructor.objects.all().count()
    print(f'Students Count: {count_of_student}\nCourses: {count_of_courses}\nInstructors Count: {count_of_instructor}')
    return complete(request)
...
def problem_five(request):
    insert_new_student = Student.objects.create(id=11, first_name="John", last_name="Lee", year="9", gpa=3.0)
    print(f'Id: {insert_new_student.id}\nFull Name: {insert_new_student.first_name} {insert_new_student.last_name}\nYear: {insert_new_student.year}\nGPA: {insert_new_student.gpa}')
    return complete(request)
...
def problem_six(request):  
    ...
    student_id = 11
    Student.objects.filter(pk=11).update(gpa=3.2)
    student_updated = Student.objects.get(pk=student_id)
    print(f'Id: {student_updated.id}\nFull Name: {student_updated.first_name} {student_updated.last_name}\nGPA: {student_updated.gpa}')
    return complete(request)
...
def bonus_problem(request):
    single_course = Course.objects.filter(instructor_id=3).count()
    single_course_2 = Course.objects.filter(instructor_id=6).count()
    single_instructor = Instructor.objects.get(pk=3)
    single_instructor_2 = Instructor.objects.get(pk=6)
    print(f'Instructor Name: {single_instructor.first_name} {single_instructor.last_name}')
    print(f'Instructor Name: {single_instructor_2.first_name} {single_instructor_2.last_name}')
    return complete(request)
